...
def package_booking(request, id):
    """Step 1: Collect booking details, create session, redirect to payment"""
    ...
    if request.method == "POST":
        ...
        if duration_days > package.duration_days:
            messages.error(request, f"This package allows maximum {package.duration_days} days only!")
            return render(request, 'package.html', {'package': package, 'user_name': request.session.get('user_name')})
        ...
        if payment_method == "online":
            ...
            messages.success(request, f'Booking confirmed successfully! Total amount: ₹{total_price} (to be paid on arrival)')
...
def payment_page(request):
    """Step 2: Show payment page with Razorpay"""
...
def cancel_booking(request, id):
    """Cancel booking - only for online paid bookings"""
    ...
    # Cash on arrival bookings cannot be cancelled
    if booking.payment_method == 'cash':
        messages.error(request, "Cash on Arrival bookings cannot be cancelled online. Please contact our support team for assistance.")
        return redirect('my_bookings')
    ...
    # Restore hotel availability if applicable
    if booking.hotel:
...
def download_booking_bill(request, booking_id):
    ...
    elements.append(Paragraph("<b>Payment Status:</b> Confirmed", normal_style))
...
def payment_verification(request):
    """Step 3: Verify payment and create booking ONLY after successful payment"""
    if request.method == "POST":
        try:
            ...
            try:
                ...
                if temp_booking['type'] == 'package':
                    ...
                    messages.success(request, f'✅ Payment successful! Your booking is confirmed. Booking ID: #{booking.id}')
                    ...
                    messages.success(request, f'✅ Payment successful! Hotel booked successfully. Booking ID: #{booking.id}')
            ...
            messages.error(request, '❌ Payment verification failed! No booking has been created.')
            if 'temp_booking' in request.session:
                del request.session['temp_booking']
            if 'razorpay_order_id' in request.session:
                del request.session['razorpay_order_id']
...
def payment_cancel(request):
    """Handle payment cancellation - NO BOOKING CREATED"""
    if 'temp_booking' in request.session:
        del request.session['temp_booking']
    if 'razorpay_order_id' in request.session:
        del request.session['razorpay_order_id']
    
    messages.warning(request, '⚠️ Payment was cancelled. No booking has been created.')
    return redirect('travels')